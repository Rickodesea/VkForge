...
static void DrawCallback(VkForgeRender render) {
    ...
    vkCmdSetViewport(render.drawCmdBuf, 0, 1, &viewport);
    vkCmdSetScissor(render.drawCmdBuf, 0, 1, &scissor);

    if(entity_count)
    {
        // if any descriptset, you need to bind them manually using Vk functions
        VkForge_BindPipeline(userData->layout, "Default", render.drawCmdBuf);
