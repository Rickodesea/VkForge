...
static void DrawCallback(VkForgeRender render)
{
    ...
    if (entity_count)
    {
        ...
        // (In your case, it seems you don't have descriptor sets for the "Default" pipeline)
...
int main()
{
    ...
    // If you had descriptor sets, you would queue them here:
    // Example (commented out since your "Default" pipeline doesn't seem to have descriptors):
    /*
    VkForgeDescriptorResource resource;
    resource.buffer.buffer = userData.entityBuffer.buffer;
    resource.buffer.offset = 0;
    resource.buffer.range = sizeof(VisualRect) * 24;
