...
)
{
	...
	VkShaderModule shader_vert = VkForge_CreateShaderModule(device, "build/default.vert.spv");
	if( VK_NULL_HANDLE == shader_vert )
	{
		SDL_LogError(0, "Failed to create vert shader for Simple pipeline\n");
		exit(1);
	}

	VkShaderModule shader_frag = VkForge_CreateShaderModule(device, "build/default.frag.spv");
	if( VK_NULL_HANDLE == shader_frag )
	{
		SDL_LogError(0, "Failed to create frag shader for Simple pipeline\n");
		exit(1);
	}
	...
	uint32_t bindingDescCount = 1;

	VkVertexInputAttributeDescription attributeDesc[] =
	{
		0
	};
	uint32_t attributeDescCount = 0;
	...
	if (result != VK_SUCCESS) {
		SDL_LogError(0, "Failed to create pipeline Simple");
...
VkPipeline VkForge_CreatePipelineForSimple2
(
    VkAllocationCallbacks* allocator,
    void* next,
    VkDevice device,
...
)
{
	...
	VkShaderModule shader_vert = VkForge_CreateShaderModule(device, "build/test.vert.spv");
	if( VK_NULL_HANDLE == shader_vert )
	{
		SDL_LogError(0, "Failed to create vert shader for Simple2 pipeline\n");
		exit(1);
	}

	VkShaderModule shader_frag = VkForge_CreateShaderModule(device, "build/test.frag.spv");
	if( VK_NULL_HANDLE == shader_frag )
	{
		SDL_LogError(0, "Failed to create frag shader for Simple2 pipeline\n");
		exit(1);
	}
	...
	VkVertexInputAttributeDescription attributeDesc[] =
	{
		...
		},
		{
			...
			.location = 1,
			.format = VK_FORMAT_R32G32_SFLOAT,
			.offset = 0 + sizeof(float) * 2,
		},
		{
			.binding = 0,
			.location = 2,
			.format = VK_FORMAT_R32G32B32A32_SFLOAT,
			.offset = 0 + sizeof(float) * 2 + sizeof(float) * 2,
		},
	};
	uint32_t attributeDescCount = 3;
	...
	result = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, allocator, &pipeline);
	if (result != VK_SUCCESS) {
		SDL_LogError(0, "Failed to create pipeline Simple2");
		return VK_NULL_HANDLE;
	}
...
VkPipeline VkForge_CreatePipelineForSimple3
(
    VkAllocationCallbacks* allocator,
    void* next,
    VkDevice device,
...
)
{
	...
	if( VK_NULL_HANDLE == shader_vert )
	{
		SDL_LogError(0, "Failed to create vert shader for Simple3 pipeline\n");
		exit(1);
	}
	...
	if( VK_NULL_HANDLE == shader_frag )
	{
		SDL_LogError(0, "Failed to create frag shader for Simple3 pipeline\n");
		exit(1);
	}
	...
	result = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, allocator, &pipeline);
	if (result != VK_SUCCESS) {
		SDL_LogError(0, "Failed to create pipeline Simple3");
		return VK_NULL_HANDLE;
	}
...
VkPipeline VkForge_CreatePipelineForSimple4
(
    VkAllocationCallbacks* allocator,
    void* next,
    VkDevice device,
...
)
{
	...
	if( VK_NULL_HANDLE == shader_vert )
	{
		SDL_LogError(0, "Failed to create vert shader for Simple4 pipeline\n");
		exit(1);
	}
	...
	if( VK_NULL_HANDLE == shader_frag )
	{
		SDL_LogError(0, "Failed to create frag shader for Simple4 pipeline\n");
		exit(1);
	}
	...
	result = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, allocator, &pipeline);
	if (result != VK_SUCCESS) {
		SDL_LogError(0, "Failed to create pipeline Simple4");
		return VK_NULL_HANDLE;
	}
